...
class StopSignPublisher(Node):
    ...
    def __init__(self):
        ...
        self-publisher_ = self.create_publisher(String, '/stop_sign', 10)  # to publish stop sign detection output (yes/no)
    ...
    def publish_detection_output(self):
        ...
        if(stop_sign_detected):
            ...
            self.logger.info("Stop sign detected! Car will stop for a while now!")
        else:
            # publish no message to subscriber if stop-sign is not detected
            msg = String()
            msg.data = "no"
            self.publisher_.publish(msg)
            self.logger.info("No stop sign detected! Car will keep moving!")
