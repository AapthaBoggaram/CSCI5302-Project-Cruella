...
class PidController(Node):
	...
	def msg_callback(self, msg):        
		...
		forward_readings = []
		reading_range = np.array([357, 358, 359, 0, 1, 2, 3])
		...
		#print('Reading Left')
		left_readings = []
		for i in range(int(one_deg*45), int(one_deg*80)): #Prior Values: for i in range(int(one_deg*10), int(one_deg*70)):
		...
		#print('Reading Right')
		right_readings = []
		for i in range(int(one_deg*280), int(one_deg*315)): #Prior Values: for i in range(int(one_deg*290), int(one_deg*350)):
			if (msg.ranges[i] < float('inf')) and not (np.isnan(msg.ranges[i])):
